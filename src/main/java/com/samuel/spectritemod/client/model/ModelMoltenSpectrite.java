package com.samuel.spectritemod.client.model;
...
public class ModelMoltenSpectrite implements IModelCustomData
{
    ...
    public static enum FluidLoader implements ICustomModelLoader
    {
        ...
        {
            return modelLocation.getResourceDomain().equals(SpectriteMod.MOD_ID) && (
    ...
    private static final class BakedFluid implements IPerspectiveAwareModel
    {
        ...
        private final LoadingCache<Long, BakedFluid> modelCache = CacheBuilder.newBuilder().maximumSize(200).build(new CacheLoader<Long, BakedFluid>()
        {
			...
			public BakedFluid load(Long key) throws Exception
            {
                ...
                boolean isOdd = key > 2048;
                return new BakedFluid(transformation, transforms, format, color, isOdd ? stillOdd : still, isOdd ? flowingOdd : flowing, gas, statePresent, cornerRound, flowRound);
            }
        });
        ...
        private TextureAtlasSprite still, flowing, stillOdd, flowingOdd;
        private final boolean gas;
        private final EnumMap<EnumFacing, List<BakedQuad>> faceQuads;

        public BakedFluid(Optional<TRSRTransformation> transformation, ImmutableMap<TransformType, TRSRTransformation> transforms, VertexFormat format, int color,
    		TextureAtlasSprite still, TextureAtlasSprite flowing, TextureAtlasSprite stillOdd, TextureAtlasSprite flowingOdd, boolean gas, Optional<IExtendedBlockState> stateOption)
        {
        ...
        public BakedFluid(Optional<TRSRTransformation> transformation, ImmutableMap<TransformType, TRSRTransformation> transforms, VertexFormat format, int color,
    		TextureAtlasSprite still, TextureAtlasSprite flowing, boolean gas, boolean statePresent,
                ...
                for(int k = 0; k < 2; k++)
                {
                    ...
                    for (int i = gas ? 3 : 0; i != (gas ? -1 : 4); i += (gas ? -1 : 1))
                    {
                        ...
                        putVertex(
                            builder, side,
                            x[l], y[l], z[l],
                            topSprite.getInterpolatedU(flow == 0 ? 8 + c * (x[l] * 2 - 1) + s * (z[l] * 2 - 1) : u[i]),
                            topSprite.getInterpolatedV(flow == 0 ? 8 + c * (x[(l + 1) % 4] * 2 - 1) + s * (z[(l + 1) % 4] * 2 - 1) : v[i]));
                    }
		...
		public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand)
        {
            ...
            if(state instanceof IExtendedBlockState)
            {
                ...
                long key = flowRound + 1024 + (exState.getValue(BlockMoltenSpectrite.ODD) ? 2048 : 0);
                for(int i = 3; i >= 0; i--)
                {
                    key <<= 10;
                    key |= cornerRound[i];
    ...
    {
        if(!customData.containsKey("molten_spectrite_fluid")) return this;

        String fluidStr = customData.get("molten_spectrite_fluid");
