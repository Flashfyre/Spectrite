...
public class EntitySpectriteWitherSkeleton extends EntityWitherSkeleton implements ISpectriteMob, ISpectriteBipedMob {
	...
	public EntitySpectriteWitherSkeleton(World worldIn) {
		...
		setBoss(!this.world.isRemote && SpectriteModConfig.spectriteMobBossSpawnRate > 0d
			&& (int) getUniqueID().getMostSignificantBits() % (100 / SpectriteModConfig.spectriteMobBossSpawnRate) == 0);
    ...
    {
		Item spectriteSword;
		boolean hasPerfectWeapon = this.boss || SpectriteModConfig.spectriteMobPerfectWeaponRate > 0d
			&& rand.nextInt((int) (100 / SpectriteModConfig.spectriteMobPerfectWeaponRate)) < chanceMultiplier;
		...
		if (!hasPerfectWeapon) {
			...
			boolean hasLegendBlade =  this.boss || SpectriteModConfig.spectriteMobLegendSwordRate > 0d
					&& rand.nextInt((int) (100 / SpectriteModConfig.spectriteMobLegendSwordRate)) < chanceMultiplier;
		...
		this.setDropChance(EntityEquipmentSlot.OFFHAND, new Double(SpectriteModConfig.spectriteMobSwordDropRate).floatValue() / (100f / chanceMultiplier));
		
		if (this.boss) {
			this.setItemStackToSlot(EntityEquipmentSlot.OFFHAND, new ItemStack(ModItems.spectrite_orb));
			this.setDropChance(EntityEquipmentSlot.OFFHAND, new Double(SpectriteModConfig.spectriteMobOrbDropRate).floatValue() / (100f / chanceMultiplier));
		}
		...
		if (this.boss || rand.nextInt(4) == 0) {
			...
			this.setDropChance(EntityEquipmentSlot.HEAD, new Double(SpectriteModConfig.spectriteMobArmourDropRate).floatValue() / (100f / chanceMultiplier));
		...
		if (this.boss || rand.nextInt(4) == 0) {
			...
			this.setDropChance(EntityEquipmentSlot.CHEST, new Double(SpectriteModConfig.spectriteMobArmourDropRate).floatValue() / (100f / chanceMultiplier));
		...
		if (this.boss || rand.nextInt(4) == 0) {
			...
			this.setDropChance(EntityEquipmentSlot.LEGS, new Double(SpectriteModConfig.spectriteMobArmourDropRate).floatValue() / (100f / chanceMultiplier));
		...
		if (this.boss || rand.nextInt(4) == 0) {
			...
			this.setDropChance(EntityEquipmentSlot.FEET, new Double(SpectriteModConfig.spectriteMobArmourDropRate).floatValue() / (100f / chanceMultiplier));
    ...
    public void addTrackingPlayer(EntityPlayerMP player)
    {
        ...
        if (this.boss) {
        	...
        	SpectriteMod.Network.sendTo(new PacketSyncSpectriteBoss(getUniqueID(), true), player);
        }
    }
